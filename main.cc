...
vector<Link> cmdInitLinks(int playerNumber, string links)
{
    ...
    {
        LinkType linkTempType =
            (links[i] == 'D' ? LinkType::Data : LinkType::Virus);
        int linkTempStrength =
            links[i + 1] - '0';
        char linkTempName =
            (playerNumber == 1 ? 'a' : 'A') + j;

        linksInit
            .emplace_back(Link(linkTempType, linkTempStrength, linkTempName));
    }

    return linksInit;
}

// main() is the command interpreter
    ...
    try
    {
        ...
        for (int i = 1; i < argc; ++i)
        {
            ...
            if (arg == "-ability1")
            {
                ...
                allAbilities
                    .emplace_back(make_pair(1, cmdInitAbilities(abilities)));
            }
            else if (arg == "-ability2")
            {
                ...
                allAbilities
                    .emplace_back(make_pair(2, cmdInitAbilities(abilities)));
            }
            else if (arg == "-link1")
            {
