...
{
    locationOfLinks = vector<vector<pair<int, int>>>(
        NUMPLAYERS,
        vector<pair<int, int>>(NUMLINKS, make_pair(-1, -1)));
    ...
    vector<vector<pair<char, string>>> displayLinks =
        vector<vector<pair<char, string>>>(
            NUMPLAYERS,
            vector<pair<char, string>>(NUMLINKS, make_pair('%', "N0")));

    vector<vector<char>> displayGrid =
        vector<vector<char>>(GRIDSIZE, vector<char>(GRIDSIZE, '&'));
        ...
        if (abilityName == Ability::Uber)
        {
            ...
            if (!linkStayedTheSame)
            {
                if (('A' <= otherCellName && otherCellName <= 'H') ||
                    ('a' <= otherCellName && otherCellName <= 'h'))
                {
                    locationOfLinks[(playerIndex + 1) % NUMPLAYERS]
                                   [linkIndex] = make_pair(-1, -1);
                }
